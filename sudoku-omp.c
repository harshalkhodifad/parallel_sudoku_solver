...
/*
To run the code for all the cases execute the run1.sh file

TO COMPILE gcc sudoku-omp.c -fopenmp -lm
TO RUN ./a.out filename threads
THIS WILL BY DEFAULT RUN THE SERIAL CODE FISRT
AND THE THE PARALLEL CODE WILL RUN ON SPECIFIED THREADS
*/
...
/*
Function: print_sudoku(MATRIX *matrix)
Description: prints a MATRIX to the  output
 */
...
/*
Function: read_sudoku(char *filename)
Description: Reads matrix from txt file
The first line is n of n^2 x n^2 matrix.
 */
...
/*
Function: possible(MATRIX matrix, short i_row, short j_column)
Description: Checks if the value at (i_row, j_col) in the matrix is valid/possible or not. Returns 1 if it is possible and 0 if it is not
 */
...
/*
Function: create_node(MATRIX matrix, short i, short j)
Description: creates an list for the matrix and returns it 
 */
...
/*
Function: push_node(MATRIX matrix, short i, short j)
Description: adds an list to the tail of the linked list 
 */
...
/*
Function: pop_node()
Description: removes an list from the head of the linked list and returns it
 */
list* pop_node()
{
	list* result = NULL;
...
/*Same functionality as create_list2 but just Improved version  */
void create_list(MATRIX* matrix)
{
	...
	while(1) {
		...
		//adding the matrix to the pool only if the value is possible
...
int main(int argc, char* argv[]) {
	...
	//BEGIN SERIAL
		...
		//sending the input matrix to the bfs_pool method which would return 1 if a solution is found
